...
import com.fasterxml.jackson.databind.node.{IntNode,NullNode}
  ...
  private def updateFields(originalFields: mutable.Map[String, Field],
    ...
    for ((key, field) <- newField) {
      if (key != "others" && (originalFields contains key)) {
        ...
        val tempField: Field = new Field(validName(newKey).get, field.schema(), field.doc(), field.defaultVal())
        for ((name, json) <- field.getObjectProps.asScala)
